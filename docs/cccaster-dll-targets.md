...
| `DllHacks.cpp` | Hack lifecycle: applies AsmLists, hooks WindowProc via MinHook (keyboard/mouse/device-change), hooks DX9 via D3DHook. Constructs `InitialGameState` + `SyncHash` (reads ~25 gameplay addresses, computes xxHash128). | ~312 | **(A)** + **(C)** | Yes (heavy) |
